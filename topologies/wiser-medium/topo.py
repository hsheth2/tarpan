...
#        redisBaseConfigPath = selfPath + '/configs/'
        ...
        quaggaHosts.append(QuaggaHost(name='d1', ip='172.0.4.1/16',
                                      loIP='10.0.4.1/24'))
        quaggaHosts.append(QuaggaHost(name='e1', ip='172.0.5.1/16',
                                      loIP='10.0.5.1/24'))
...
#        testHost = self.addHost(name='test')
    ...
    def startup_commands(self, hosts):
        hosts[3].cmdPrint("/usr/local/vanilla_quagga/sbin/bgpd --daemon -A 127.0.0.1 -f /etc/quagga/bgpdmeow.conf")
