...
attrhash_cmp (const void *p1, const void *p2)
{
  ...
  if (attr1->flag == attr2->flag && attr1->origin == attr2->origin
      ...
      && attr1->aspath == attr2->aspath && attr1->community == attr2->community
      && attr1->tarpan == attr2->tarpan && attr1->med == attr2->med
      && attr1->local_pref == attr2->local_pref)
    {
      ...
      if (ae1 && ae2
	  && ae1->aggregator_as
	      == ae2->aggregator_as && ae1->aggregator_addr.s_addr == ae2->aggregator_addr.s_addr
...
{
  hash_iterate (attrhash, (void
  (*) (struct hash_backet *, void *)) attr_show_all_iterator,
		vty);
}
		    ...
		    bgp_size_t length)
{
  ...
  stream_set_getp (
      BGP_INPUT(peer),
      (args->startp - STREAM_DATA(BGP_INPUT (peer))) + args->total);
   ...
   */
  if (CHECK_FLAG(
      flags, BGP_ATTR_FLAG_TRANS) && CHECK_FLAG (flags, BGP_ATTR_FLAG_OPTIONAL)
...
const u_int8_t attr_flags_values[] =
      { [BGP_ATTR_ORIGIN] = BGP_ATTR_FLAG_TRANS, [BGP_ATTR_AS_PATH
	  ] = BGP_ATTR_FLAG_TRANS, [BGP_ATTR_NEXT_HOP] = BGP_ATTR_FLAG_TRANS,
	  ...
	  [BGP_ATTR_EXT_COMMUNITIES] = BGP_ATTR_FLAG_OPTIONAL
	      | BGP_ATTR_FLAG_TRANS, [BGP_ATTR_AS4_PATH
	      ] = BGP_ATTR_FLAG_OPTIONAL | BGP_ATTR_FLAG_TRANS,
	  [BGP_ATTR_AS4_AGGREGATOR] = BGP_ATTR_FLAG_OPTIONAL
	      | BGP_ATTR_FLAG_TRANS, };
static const size_t attr_flags_values_max = sizeof(attr_flags_values)
    / sizeof(attr_flags_values[0]);
...
bgp_attr_flag_invalid (struct bgp_attr_parser_args *args)
{
   ...
   * 1."
   */
  if (!CHECK_FLAG(BGP_ATTR_FLAG_OPTIONAL,
		  flags) && !CHECK_FLAG (BGP_ATTR_FLAG_TRANS, flags))
    {
  ...
  if (CHECK_FLAG(flags, BGP_ATTR_FLAG_PARTIAL))
	...
	{
	  zlog (peer->log, LOG_ERR, "%s well-known attribute "
      ...
      if (CHECK_FLAG(
	  flags,
	  BGP_ATTR_FLAG_OPTIONAL) && !CHECK_FLAG (flags, BGP_ATTR_FLAG_TRANS))
	{
	  zlog (peer->log, LOG_ERR, "%s optional + transitive attribute "
		"must NOT have the partial flag set (%x)",
		LOOKUP(attr_str, attr_code), flags);
	  return 1;
	}
   ...
   */
  if (CHECK_FLAG(
      flags, BGP_ATTR_FLAG_OPTIONAL) && CHECK_FLAG (flags, BGP_ATTR_FLAG_TRANS))
...
bgp_attr_aspath (struct bgp_attr_parser_args *args)
{
  ...
  if (!attr->aspath)
    {
      zlog (peer->log, LOG_ERR, "Malformed AS path from %s, length is %d",
	    peer->host, length);
...
bgp_attr_aspath_check (struct peer * const peer, struct attr * const attr)
{
  ...
  if (bgp != NULL && bgp_flag_check (bgp, BGP_FLAG_ENFORCE_FIRST_AS))
	...
	{
	  zlog (peer->log, LOG_ERR, "%s incorrect first AS (must be %u)",
		peer->host, peer->as);
	  bgp_notify_send (peer, BGP_NOTIFY_UPDATE_ERR,
	  BGP_NOTIFY_UPDATE_MAL_AS_PATH);
	  return BGP_ATTR_PARSE_ERROR;
...
bgp_attr_as4_path (struct bgp_attr_parser_args *args, struct aspath **as4_path)
{
  ...
  *as4_path = aspath_parse (peer->ibuf, length, 1);
  ...
  if (!*as4_path)
    {
      zlog (peer->log, LOG_ERR, "Malformed AS4 path from %s, length is %d",
	    peer->host, length);
      return bgp_attr_malformed (args,
      BGP_NOTIFY_UPDATE_MAL_AS_PATH,
				 0);
...
bgp_attr_nexthop (struct bgp_attr_parser_args *args)
{
  ...
  if (IPV4_NET0(
      nexthop_h) || IPV4_NET127 (nexthop_h) || IPV4_CLASS_DE (nexthop_h))
    {
...
bgp_attr_community (struct bgp_attr_parser_args *args)
{
  ...
  attr->community = community_parse ((u_int32_t *) stream_pnt (peer->ibuf),
				     length);
...
bgp_attr_originator_id (struct bgp_attr_parser_args *args)
{
  ...
  (bgp_attr_extra_get (attr))->originator_id.s_addr = stream_get_ipv4 (
      peer->ibuf);
...
bgp_attr_cluster_list (struct bgp_attr_parser_args *args)
{
  ...
  (bgp_attr_extra_get (attr))->cluster = cluster_parse (
      (struct in_addr *) stream_pnt (peer->ibuf), length);
...
#define LEN_LEFT	(length - (stream_get_getp(s) - start))
  if ((length > STREAM_READABLE(s)) || (length < BGP_MP_REACH_MIN_SIZE))
    {
      zlog_info ("%s: %s sent invalid length, %lu", __func__, peer->host,
...
#endif /* HAVE_IPV6 */
    default:
      zlog_info ("%s: (%s) Wrong multiprotocol next hop length: %d", __func__,
		 peer->host, attre->mp_nexthop_len);
      return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
    }
  ...
  if (safi != SAFI_MPLS_LABELED_VPN)
    {
      ...
      if (ret < 0)
	{
	  zlog_info ("%s: (%s) NLRI doesn't pass sanity check", __func__,
		     peer->host);
	  return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
	}
    }
...
bgp_attr_ext_communities (struct bgp_attr_parser_args *args)
{
  ...
  (bgp_attr_extra_get (attr))->ecommunity = ecommunity_parse (
      (u_int8_t *) stream_pnt (peer->ibuf), length);
...
bgp_tarpan_parse (struct bgp_attr_parser_args *args)
{
  ...
  attr->tarpan = tarpan_parse ((u_int8_t *) stream_pnt (peer->ibuf), length);
...
bgp_attr_check (struct peer *peer, struct attr *attr)
{
  ...
  if (!CHECK_FLAG(
      attr->flag,
      ATTR_FLAG_BIT (BGP_ATTR_NEXT_HOP)) && !CHECK_FLAG (attr->flag, ATTR_FLAG_BIT (BGP_ATTR_MP_REACH_NLRI)))
    type = BGP_ATTR_NEXT_HOP;

  if (peer->sort
      == BGP_PEER_IBGP && ! CHECK_FLAG (attr->flag, ATTR_FLAG_BIT (BGP_ATTR_LOCAL_PREF)))
    type = BGP_ATTR_LOCAL_PREF;

  if (type)
    {
      zlog (peer->log, LOG_WARNING, "%s Missing well-known attribute %d / %s",
	    peer->host, type, LOOKUP(attr_str, type));
      bgp_notify_send_with_data (peer,
      BGP_NOTIFY_UPDATE_ERR,
				 BGP_NOTIFY_UPDATE_MISS_ATTR, &type, 1);
      return BGP_ATTR_PARSE_ERROR;
    }
  return BGP_ATTR_PARSE_PROCEED;
}
		...
		struct bgp_nlri *mp_update, struct bgp_nlri *mp_withdraw)
{
  ...
  while (BGP_INPUT_PNT (peer) < endp)
	...
	{ .peer = peer, .length = length, .attr = attr, .type = type, .flags =
	    flag, .startp = startp, .total = attr_endp - startp, };
	...
	{
	  zlog (peer->log, LOG_WARNING, "%s: Attribute %s, parse error",
	...
	{
	  zlog (peer->log, LOG_WARNING, "%s: BGP attribute %s, fetch error",
		peer->host, LOOKUP(attr_str, type));
	  bgp_notify_send (peer,
	  BGP_NOTIFY_UPDATE_ERR,
			   BGP_NOTIFY_UPDATE_ATTR_LENG_ERR);
  ...
  if (BGP_INPUT_PNT (peer) != endp)
    {
      zlog (peer->log, LOG_WARNING, "%s: BGP attribute %s, length mismatch",
	    peer->host, LOOKUP(attr_str, type));
      bgp_notify_send (peer,
      BGP_NOTIFY_UPDATE_ERR,
		       BGP_NOTIFY_UPDATE_ATTR_LENG_ERR);
  ...
  if (!CHECK_FLAG(attr->flag, ATTR_FLAG_BIT (BGP_ATTR_TARPAN)))
	...
	{
	  zlog_info (
	      "Calling tarpan api packet_received_handler on default tarpan attribute");
	  tarpan_active_handler->packet_received_handler (peer, attr);
	}
    }
...
int
stream_put_prefix (struct stream *, struct prefix *);
...
bgp_packet_mpattr_prefix (struct stream *s, afi_t afi, safi_t safi,
			  struct prefix *p, struct prefix_rd *prd, u_char *tag)
{
  switch (safi)
    {
    case SAFI_MPLS_VPN:
...
bgp_packet_attribute (struct bgp *bgp, struct peer *peer, struct stream *s,
		      struct attr *attr, struct prefix *p, afi_t afi,
...
//      SET_FLAG(attr->flag, ATTR_FLAG_BIT (BGP_ATTR_TARPAN));
  ...
  assert(tarpan_wire);
  stream_putc (
      s, BGP_ATTR_FLAG_OPTIONAL | BGP_ATTR_FLAG_TRANS | BGP_ATTR_FLAG_EXTLEN); // we always use extended length
...
void
bgp_packet_mpunreach_prefix (struct stream *s, struct prefix *p, afi_t afi,
